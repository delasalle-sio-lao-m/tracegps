...
{	$msg = "Erreur : méthode HTTP incorrecte.";
    ...
    if (($pseudo == '' || $mdpSha1 == '' || $idTrace == '' || $dateHeure == '' || $lang == '' || $latitude == '' || $longitude == '' || $altitude == '' || $rythmeCardio == '') ) {
    	    ...
    	    if ( $uneTrace == null ) {
    	        ...
    	        if ( $uneTrace->getIdUtilisateur() != $unUtilisateur->getId() ) {
    			    ...
    			    if ( $uneTrace->getTerminee() == false ) {
    				    ...
    				    $uneVitesse = 0;
    				    $idPoint = $nbPoint + 1;
    				    $unPoint = new PointDeTrace($idTrace, $idPoint, $latitude, $longitude, $altitude, $uneDateHeure, $rythmeCardio, $unTempsCumule, $uneDistanceCumulee, $uneVitesse);
...
{	
	...
	if($idPoint != 0){
	    ...
	    $elt_id = $doc->createElement('id');
	    $elt_donnees->appendChild($elt_id);
	    
	    $elt_idPoint= $doc->createElement('id', $idPoint);
	    $elt_id->appendChild($elt_idPoint);
	}
