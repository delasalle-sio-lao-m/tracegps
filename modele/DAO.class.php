...
class DAO
{
    ...
    public function getLesUtilisateursAutorises($idUtilisateur) {
        // préparation de la requête de recherche
        $txt_req = "Select id, pseudo, mdpSha1, adrMail, numTel, niveau, dateCreation, nbTraces, dateDerniereTrace";
        $txt_req .= " from tracegps_vue_utilisateurs, tracegps_autorisations, tracegps_utilisateurs";
        $txt_req .= " where tracegps_utilisateurs.id = tracegps_autorisations.idAutorisant";
        $txt_req .= " where idAutorisant = :id";
        // $txt_req .= " where idAutorisant = :id";
        $req = $this->cnx->prepare($txt_req);
        // liaison de la requête et de ses paramètres
        $req->bindValue("id", $idUtilisateur, PDO::PARAM_STR);
        // extraction des données
        $req->execute();
        $uneLigne = $req->fetch(PDO::FETCH_OBJ);
        // construction d'une collection d'objets Utilisateur
